...
def make_test_vector_regions(hdu, x0, y0, x1, y1):
    ...
    length = point1.separation(point2).to(u.arcsec).value
    ...
    dy = y1 - y0
    dx = x1 - x0
    theta = [math.atan2(dy[i], dx[i]) * u.rad.to(u.deg) for i in range(len(dx))]
...
def make_regions(sources, hdu, outfile, coordsys=False, coordheads=False, reg_type='circle', additional_coords=False, coordnames=False, idheader=False, color="#FFC107", radius=False, radunit=False, label=False, width=1, fontsize=10, bold=False, dash=False, addshift=[0,0], savecoords=None, marker=False, fill=False): 
    ...
    if isinstance(sources, list): # if sources is a list, assume they are a list of coordinates
        ...
        try: 
            ...
            xcoord,ycoord = coordheads.split(",")
            # If this still fails, then user has erred and program will end in error. 
            x_coords = sourcesc.values.tolist()
            y_coords = sources[ycoord].values.tolist()
    ...
    if reg_type.lower() == 'point': 
        ...
        x1_coords = sources[additional_coords[0]]
        y1_coords = sources[additional_coords[1]]
        reg, reg_props, length, theta = make_test_vector_regions(hdu, x_coords, y_coords, 
                                                            x1_coords, y1_coords)
    else:
        raise ValueError(f"{reg_type} invalid region type.")
        # ValueError.add_note(f"{reg_type} invalid region type.")
